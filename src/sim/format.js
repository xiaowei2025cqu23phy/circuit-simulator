...
export const TOOL_HINTS = {
  select: '选择/探测 (Q)：点击元件查看属性与波形；拖拽可移动；仿真中点击手动开关切换',
  move: '移动 (M)：拖拽元件整体移动；拖拽端点可重新接线',
  wire: '导线 (W)：按下拖拽绘制；正交模式下自动直角拐弯；靠近端点自动吸附',
  resistor: '电阻 (R)：按下拖拽绘制，选中后在左侧面板设置阻值',
  capacitor: '电容 (C)：按下拖拽绘制，参数单位法拉 (F)',
  inductor: '电感 (L)：按下拖拽绘制，参数单位亨利 (H)',
  diode: '二极管 (D)：三角形一端为阳极 (A)，竖线一端为阴极 (K)，导通压降 0.7V',
  switch: '开关 (S)：手动开关仿真中点击切换；定时开关按设定时刻自动通断',
  voltage: '电压源 (V)：支持 DC / AC / 方波 / 三角波 / 脉冲 / 指数 / 阶跃波形',
  current: '电流源 (I)：支持与电压源相同的波形类型',
  terminal: '接线端子 (T)：命名端子可跨区域连接同名网络；命名为 GND 即接地',
  ground: '接地 (G)：电路必须包含接地端才能仿真',
  label: '网络标签 (N)：相同名称的标签电气相连，NET 为默认名',
  delete: '删除 (X)：点击元件将其移除',
};
